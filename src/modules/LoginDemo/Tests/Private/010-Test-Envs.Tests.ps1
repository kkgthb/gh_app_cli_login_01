...
Describe "Testing script internals -- it should validate environment variables" {
    InModuleScope 'MyEphemeralModule' {
        It "validates proper variables set" {
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_pem', 'HelloWorldPem', 'Process')
            ...
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_client_id', 'Iv23liABCDEFGHIJKLMN', 'Process')
            { Test-Envs } | Should -Not -Throw
        }
        It "catches missing org name" {
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_pem', $null, 'Process')
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_id', '12345', 'Process')
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_installation_id', '54321', 'Process')
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_client_id', 'Iv23liABCDEFGHIJKLMN', 'Process')
            { Test-Envs } | Should -Throw 'Required environment variable DEMOS_my_gh_app_pem is not set.'
        }
        ...
        It "catches missing client ID" {
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_pem', 'HelloWorldPem', 'Process')
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_id', '12345', 'Process')
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_installation_id', '54321', 'Process')
            [Environment]::SetEnvironmentVariable('DEMOS_my_gh_app_client_id', $null, 'Process')
            { Test-Envs } | Should -Throw 'Required environment variable DEMOS_my_gh_app_client_id is not set.'
        }
    }
}
